// Pre-optimization transition oracle. Keep independent of production logic.
    function automatic entry_t reference_completion(input entry_t e,input message_t m);
        entry_t result;
    ...
    function automatic entry_t reference_row_update(input entry_t previous, input logic [6:0] slot);
        entry_t next_entry;
        ...
        if(!rst_n || flush) next_entry='0;
            ...
            if(next_entry.valid) begin
                ...
                if(complete_now) next_entry=reference_completion(next_entry,done_message);
                for(k=0;k<4;k++) begin
