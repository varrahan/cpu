#!/usr/bin/env python3
"""Netlist-based CDC/RDC checks for interrupt and JTAG clock domains."""
...
def main():
    top = load("build/cdc/top.json", "top")
    top_cone = combinational_cones(top)
    core_clock = top["ports"]["clk"]["bits"][0]
    interrupt_names = (
        "irq_m_software", "irq_m_timer", "irq_m_external",
        "irq_s_software", "irq_s_timer", "irq_s_external", "nmi",
    )
    allowed = set()
    async_bits = set()
    for name in interrupt_names:
        source = top["ports"][name]["bits"][0]
        async_bits.add(source)
        allowed.add(id(synchronizer(
            top, f"{name}_sync", source, core_clock, top_cone
        )))
    for cell in top["cells"].values():
        if sequential(cell):
            if cell["type"] == "$adff":
                raise AssertionError("core contains an asynchronous reset register")
            if any(top_cone(bit) & async_bits for bit in cell["connections"]["D"]):
                assert id(cell) in allowed, "interrupt bypasses its synchronizer"
    ...
    print(
        "PASS: elaborated CDC/RDC analysis: 9 two-flop synchronizers, "
        f"{len(async_reset_cells)} TAP-only async-reset banks, synchronous core reset"
    )
