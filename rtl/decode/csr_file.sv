module csr_file #(parameter RETIRE_WIDTH = 1) (
    input logic clk, input logic rst_n,
    input logic [11:0] read_addr, input logic read_write_intent,
    output logic [31:0] read_data, output logic [31:0] read_modify_data,
    output logic read_illegal,
    input logic commit_valid, input logic [11:0] commit_addr,
    input logic [31:0] commit_data, input logic [RETIRE_WIDTH-1:0] retire,
    output logic [31:0] commit_visible_data,
    input logic fp_flags_valid, input logic [4:0] fp_flags,
    input logic fp_dirty,
    input logic [63:0] mtime,
    input logic irq_m_software, input logic irq_m_timer,
    input logic irq_m_external, input logic irq_s_software,
    input logic irq_s_timer, input logic irq_s_external, input logic nmi,
    input logic trap_enter, input logic [31:0] trap_pc,
    input logic [31:0] trap_cause, input logic [31:0] trap_tval,
    input logic mret, input logic sret,
    output logic [31:0] trap_vector,
    output logic [31:0] return_pc, output logic [31:0] sreturn_pc,
    output logic [2:0] frm_out, output logic fp_enabled,
    output logic interrupt_pending, output logic [31:0] interrupt_cause,
    output logic wfi_wake_pending,
    output logic [1:0] privilege,
    output logic [31:0] satp_out, output logic mstatus_mprv_out,
    output logic mstatus_sum_out, output logic mstatus_mxr_out,
    output logic [31:0] pmpcfg0_out, output logic [31:0] pmpaddr0_out,
    output logic [31:0] pmpaddr1_out, output logic [31:0] pmpaddr2_out,
    output logic [31:0] pmpaddr3_out,
    output logic [1:0] data_privilege,
    output logic mret_allowed, output logic sret_allowed,
    output logic sfence_allowed, output logic wfi_allowed
);
    localparam [1:0] PRIV_U = 0, PRIV_S = 1, PRIV_M = 3;

    logic [1:0] current_privilege;
    logic mstatus_mie, mstatus_mpie, mstatus_sie, mstatus_spie;
    logic mstatus_spp, mstatus_mprv, mstatus_sum, mstatus_mxr;
    logic mstatus_tvm, mstatus_tw, mstatus_tsr;
    logic [1:0] mstatus_mpp, mstatus_fs;
    logic [31:0] medeleg, mideleg, mie;
    logic mip_ssip, mip_stip, mip_seip;
    logic [31:0] mtvec, stvec, mscratch, sscratch;
    logic [31:0] mepc, sepc, mcause, scause, mtval, stval;
    logic [31:0] mcounteren, scounteren, satp;
    ...
    logic csr_exists, csr_read_only, trap_to_s;
    logic [31:0] raw_read_data, visible_commit_data, mip_value;
    integer i;

    wire [1:0] visible_fs = fp_dirty || fp_flags_valid ? 2'b11 :
                            mstatus_fs;
    wire [63:0] visible_minstret = minstret + {{(64-RETIRE_WIDTH){1'b0}}, retire};
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (retire && !mcountinhibit[2]) minstret <= minstret + retire;
